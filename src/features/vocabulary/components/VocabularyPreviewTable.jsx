  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button variant="secondary" disabled={disabled} onClick={() => onChangeRows([...rows, createEmptyVocabularyImportRow()])}>
              ...
              return (
                <tr key={rowIndex} className={hasErrors ? 'bg-warning/5' : 'bg-card'}>
                  ...
                  {columns.map((column) => (
                    <td key={column.key} className="px-3 py-3 align-top">
                      <input
                        ...
                        className={`h-10 w-full rounded-xl border bg-background px-3 text-foreground outline-none transition placeholder:text-muted-foreground focus:border-ring focus:ring-2 focus:ring-ring/20 disabled:opacity-60 ${errors[column.key] ? 'border-destructive' : 'border-input'}`}
                  ...
                  <td className="px-3 py-3 align-top text-right">
                    <Button variant="ghost" className="h-10 w-10 px-0" disabled={disabled} onClick={() => removeRow(rowIndex)} aria-label={`Remove row ${rowIndex + 1}`}>
      ...
      {!rows.length ? (
        <div className="rounded-card border border-dashed border-border bg-muted/40 p-6 text-center text-sm text-muted-foreground">
